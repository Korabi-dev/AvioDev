...
module.exports = {
    ...
    run: async(client, message, args) =>{
        ...
        if(d1){
            if(d1.passive == true) return message.reply(client.embed("Error", "You are in passive mode.")) 
            if(d1.wallet < 500) return message.reply(client.embed("Error", "You need to have $500+ to rob someone.")) 
        }else {
        ...
        if(d){
        if(d.passive == true) return message.reply(client.embed("Error", "This user is in passive mode."))    
        ...
        } else if(chance >= 50 || message.isOwner == ftrue) {
            d.wallet = d.wallet - p
