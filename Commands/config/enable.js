module.exports = {
    ...
    run: async(client, message, args) => {
        if(!args[0]) return message.reply(client.embed("Error", "You need to provide a command to enable."))
        ...
        if(!command) {
            return message.reply(client.embed("Error", "Invalid Command."), {mention: true})
                ...
                if(data){
                    if (!data.disabled.includes(commandName)) return message.reply(client.embed("Error", "This command is already enabled."))
                    const index = data.disabled.indexOf(commandName)
                    if(index > -1){
                        data.disabled.splice(index, 1)
                    }
                    await data.save()
                    return message.reply(client.embed("Success", `The command \`${commandName}\` was enabled.`))
                } else {
                    return message.reply(client.embed("Error", "This command is already enabled."))
                }
 }
    }
}