...
class AntiSpamClient extends EventEmitter {
	...
	async banUser (message, member, spamMessages) {
		...
		if (!member.bannable) {
			...
			await message.member.ban({
				reason: 'Spamming after being kicked and rejoining.',
	...
	async muteUser (message, member, spamMessages) {
        ...
        message.member.roles.cache.forEach(async(role) => {
         await message.member.roles.remove(role, "Spamming.")
        })
		await message.member.roles.add(role, 'Spamming.').catch(e => {return;})
