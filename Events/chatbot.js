...
module.exports = {
    ...
    run: async(message, client) => {
        ...
        if(data.enabled == true && data.channel == message.channel.id){
            ...
            try{
                const { data } = await axios.get(`https://api.affiliateplus.xyz/api/chatbot?message=${message.content}&botname=${client.user.username}&ownername=Korabi&user=${message.author.id}`)
                reply = data
            }catch(e){
                console.log(e)
